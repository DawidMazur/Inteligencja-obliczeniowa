...
epsilon_decay = 0.998  # Współczynnik zmniejszania epsilon po każdym kroku czasowym
...
files = glob.glob(version_name + "/generator_model_*.keras")
if len(files) > 0:
    files.sort()
...
def train_dqn():
    ...
    if model_trains % save_model_every == 0:
        ...
        model.save(version_name + "/" + filename)
        global start_time
...
for episode in range(max_games_number):
    ...
    while not terminated:
        ...
        print(reward, terminated, info, total_reward)
